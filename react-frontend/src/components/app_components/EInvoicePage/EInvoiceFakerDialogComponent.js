...
import EInvoiceFakerFactory from "./EInvoiceFakerFactory";
...
const EInvoiceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = EInvoiceFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("EInvoice").create(elem));
...
export default connect(mapState, mapDispatch)(EInvoiceFakerDialogComponent);
